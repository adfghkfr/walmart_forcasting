...
library(FactoMineR)
require("Gifi")
data<-read.csv("/Users/guoyixuan/Documents/Code_R/PROJECT_Walmart Sales Foresasting/data/train_fin.csv")
tail(data)
dim(data)
str(data)
library(ISLR)
...
data$month<-match(data$Month,month.abb) #change to 2022-04
data
tail(data)
...
data3<-subset(data3,sell!="0") ##?R???٨S?}?l?檺????
dim(data3)
#HCPC
...
##?p???B?zitem_id?(?@??3049?????O)
##????one hot encoding?B?z???L???O?????ƦA????item id
##????:one hot encoding+kmeans?|?ɭP???ƫܤjkmeans?B???ܺC/
##k-prototype/dbscan?Ω?SVR?w???P???B
...
library(clustMixType)
#???l????
#?ѩ?item_id?ӹL?????A?ڭ̭p?⤣?Pitem_id?U???P???B???????ƨӶi???��N
###target encoding
...
###?N?w???ҫ???train???ƶ??A?H???��Φ?train??validation?ΥH?إߤ????ҫ?###
library(e1071)
...
###?إߤ????ҫ?(?ΥH?P?_validation/test???ݸs??k=1,2,..6)###
##model clus1##
library(xgboost)
library(Matrix)
library(data.table)
